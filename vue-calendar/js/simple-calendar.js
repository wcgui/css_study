...
var LunarHelp = function() {
	function LunarHelp(year, month, day,options) {
	    ...
	    var monCyl = 14
	   for(i=1900; i<options.timeRange.endYear && offset>0; i++) {
...
var SimpleCalendar = function() {
	...
	_createClass(SimpleCalendar, [{
		...
		value: function create() {
			...
			root.innerHTML = '<div class="sc-header"></div><div class="sc-body"></div><div class="sc-announcement"><ul class="sc-matter"></ul></div>';
			root.className = 'sc-calendar';
			root.style.width = this._options.width;
			root.style.height = this._options.height;
			var sheader = root.querySelector('.sc-header');
			var scbody = root.querySelector('.sc-body'),
				string = "";
				
			string += '<div class="sc-title">';
			string += '	<div class="year"><div class="arrow sc-yleft" title="上一年"></div><div class="yeardiv"><select class="sc-select-year" name=""></select>年</div><div class="arrow sc-yright" title="下一年"></div></div>';
			string += '	<div class="month">';
			string += '		<div class="arrow sc-mleft" title="上一月"></div>';
			string += '		<div class="monthdiv">';
			string += '			<select class="sc-select-month" name=""></select>';
			string += '		</div>';
			string += '		<div class="arrow sc-mright" title="下一月"></div>';
			string += '	</div></div><div class="sc-week"></div> ';
			sheader.innerHTML = string;
		...
		value: function addData(year, month) {
			...
			for(var i = 0; i < 42; i++) {
				...
				if(this._options.showLunarCalendar) {
					daysElement[i].querySelector('.lunar-day').innerHTML = new LunarHelp(writeyear, writemonth, writeday,this._options).getLunarDayName();
				} else {
			...
			if(options.showMark) {
				...
				if(data) {
					daysElement.forEach(function(v, i) {
						...
						var currentmonth = count == 0 ? month - 1 : count == 1 ? month : month+1;
						...
						if(data[currentyear + '-' + currentmonth + '-' + day]) {
							var length = options.mark[currentyear + '-' + currentmonth + '-' + day].length;
							v.querySelector('.mark').classList.remove('sc-mark-hide');
							v.querySelector('.mark').classList.add('sc-mark-show');
							v.title = data[currentyear + '-' + currentmonth + '-' + day];
						} else {
    ...
    value: function updateFestival(year, month) {
      ...
      daysElement.forEach(function (v, i) {
        ...
        if (lunardata && options.showLunarFestival) {
          var lunar = new LunarHelp(year, currentmonth, day,options).getLunarDayNum();
		...
		value: function updateEvent() {
		    ...
		    monthsub.onclick = function () {
		        ...
		        if (currentmonth > 1) {
					...
					if (--currentyear-1 < calendar._options.timeRange.startYear) {
						return false;
					}
					currentmonth = 12;
					selectYear.value = currentyear;
			...
			calendar.announceList($(container.querySelector('.sc-today')));
			$(container).on("click", '.sc-selected', function() {
				calendar.announceList($(this));
			});
		}
	},
	{//添加标记
	...
	{	
		key: "announceList",
		value: function announceList(v){
			var container = this.container;		
			var matterHtml=''

			if(v.children().hasClass('sc-mark-show')){
				var year = container.querySelector('.sc-select-year').value;
				var month = container.querySelector('.sc-select-month').value;
				...
				var content = this._options.mark[date];
				
				if(typeof(content) == 'undefined') {
					return false;
				}
				for(var i=0;i<content.length;i++){
					matterHtml +='<li class="announceItem"><div><div class="fl announceImg"></div>'
						+'<p class="announceContent" data-type="eventDetails" data-event="'+content[i].id+'">'+content[i].title+'</p>'
			...
			container.querySelector('.sc-matter').innerHTML = matterHtml;
		}
	},
	{
		//获取用户点击的日期
